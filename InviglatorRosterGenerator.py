...
def review_page():
    ...
    f0.propagate(0)
    f0.place(x=15, y=100)
    lbl_img = Label(fr_review, image=img_temp)
    lbl_img.place(x=550, y=200)
    lbl_tv5 = Label(fr_review, bg='white')
    lbl_tv5.place(x=390, y=120)
...
def analysis_page():
    ...
    ORDER BY assign_db.CourseID;""", db_conn1)
        out_path = "output\\output.xlsx"
        scheduler_df.to_excel(out_path, index=False)
